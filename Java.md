...
![image-20250607143642212](Java.assets/image-20250607143642212.png)

| 方法类型     | 代表方法                     | 功能说明                      | 记忆技巧         |
| ------------ | ---------------------------- | ----------------------------- | ---------------- |
| `parse` 类   | `Integer.parseInt("123")`    | **解析字符串为基本类型**      | parse = 解析     |
| `toXXX()` 类 | `toArray()`, `toCharArray()` | **把对象转换为其他类型/结构** | to = 转换为      |
| `valueOf()`  | `String.valueOf(123)`        | **静态转换为字符串**          | valueOf = 得到值 |

通用写法String s3 = String.valueOf(num); 
